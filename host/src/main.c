...
cl_mem check_clCreateBuffer(cl_context ctx, cl_mem_flags flags, 
...
void check_clEnqueueNDRangeKernel(cl_command_queue queue, cl_kernel k, 
...
void check_clEnqueueReadBuffer(cl_command_queue queue, cl_mem buffer, 
...
void print_device_info(unsigned id, cl_device_id dev)
{
    char	name[1024];
    size_t	len = 0;
    int		status;
    status = clGetDeviceInfo(dev, CL_DEVICE_NAME, sizeof(name), &name, &len);
    if (status != CL_SUCCESS)
...
void examine_hashtable(unsigned round, cl_command_queue queue,
        cl_mem buf_ht, uint32_t test_num)
{
    uint8_t  *hash_tb;
    if (verbose < 3)
	     return;
    hash_tb = (uint8_t *)malloc(HT_SIZE);
    if (!hash_tb)
        fatal("malloc: %s\n", strerror(errno));
    cl_event readEvent;
    check_clEnqueueReadBuffer(queue, buf_ht, CL_FALSE, 0, 
            HT_SIZE, hash_tb, 0, NULL, &readEvent);
    clWaitForEvents(1, &readEvent);
    uint8_t *p;
    printf("Hash table test:\n");
    for (uint i = 0; i <= (test_num & 0xfffff); ++i) {
        p = hash_tb + i * NR_SLOTS * SLOT_LEN;
        for (uint j = 0; j < NR_SLOTS * SLOT_LEN; ++j) {
            if (j % 8 == 0)
                printf(" ");
            if (j % 32 == 0)
                // printf(" ");
...
void examine_rowCounters(unsigned round, cl_command_queue queue, 
        cl_mem buf_rowCounters)
{
    uint32_t *rowCounters;
    if (verbose < 2)
	     return;
    rowCounters = (uint32_t *)malloc(sizeof(uint32_t) * (1 << 17)); 
    if (!rowCounters)
	    fatal("malloc: %s\n", strerror(errno));
    cl_event readEvent;
    check_clEnqueueReadBuffer(queue, buf_rowCounters, CL_FALSE, 0, 
            sizeof(uint32_t) * (1 << 17), rowCounters, 0, NULL, &readEvent);
    clWaitForEvents(1, &readEvent);
    uint num[16];
    for (uint i = 0; i < 16; ++i)
        num[i] = 0;
    for (uint i = 0; i < (1 << 17); ++i) {
        for (uint j = 0; j < 8; ++j) {
            num[(rowCounters[i] >> (j * 4)) & 0xf]++;
        }
    }
    uint sum = 0;
    printf("round%d rowCounters[%d]:\n", round, round % 2);
    for (uint i = 0; i < 16; ++i) {
        printf(" %d: %d\n", i, num[i]);
        sum += num[i] * i;
    }
    uint miss_num = 0;
    miss_num = (1 << 21) - sum;
    printf("Collision num: %d (all) %d (missed)\n", sum, miss_num);
    clReleaseEvent(readEvent);
    free(rowCounters);
    /* uint64_t loadCounters[2] = {0, 0};
    for (uint i = 2; i < 11; ++i) {
        loadCounters[0] += i * (i - 1) * num[i];
        loadCounters[1] += i * num[i];
    }
    loadCounters[0] += 120 * num[12];
    loadCounters[1] += 12 * num[12];
    printf("original load counters: %lu\nopt load counters: %lu\nreduce counters: %lu\n",
            loadCounters[0], loadCounters[1], loadCounters[0] - loadCounters[1]);
    printf("reduce to %.2lf of origin\n", (double)loadCounters[1] / loadCounters[0]);
    printf("store counters: %lu\n\n", loadCounters[0] / 2); */
}

void init_ht(cl_command_queue queue, cl_kernel k_init_ht,
        cl_mem rowCounters, cl_event *event)
{
    ...
    status = clSetKernelArg(k_init_ht, 0, sizeof(rowCounters), &rowCounters);
    if (status != CL_SUCCESS)
	   fatal("clSetKernelArg (%d)\n", status);
...
void print_sol(uint32_t *values, uint64_t *nonce, int mode)
{
...
/*
** Read a complete line from stdin. If 2 or more lines are available,
** store only the last one in the buffer.
**
...
void mining_parse_job(char *str, uint8_t *target, size_t target_len,
        char *job_id, size_t job_id_len, uint8_t *header, 
        size_t header_len, size_t *fixed_nonce_bytes)
{
    uint32_t		str_i, i;
    // parse target
    str_i = 0;
        ...
        cl_mem buf_dbg, size_t dbg_size, uint8_t *header, cl_mem *rowCounters)
{
    ...
    uint64_t    t0 = 0, t1;
    uint64_t    status_period = 500e3; // time (usec) between status 
    for (i = 0; ; ++i) {
...
** 			        incremented by the number of devices available on
** 			        this platform
        ...
        cl_platform_id *plat_id, cl_device_id *dev_id)
{
    ...
    while (i < nr_devs) {
        ...
        else if(*nr_devs_total == device_to_use) {
            found = 1;
            *plat_id = plat;
            *dev_id = devices[i];
            break;
...
void init_and_run_opencl(uint8_t *header, size_t header_len)
{
    ...
    binaries[0] = loadBinaryFile(BINARY_NAME, &lenght); 
    if (binaries[0] == NULL) 
        fatal("Failed to load binary file\n");
    cl_program program = clCreateProgramWithBinary(context, 1, &dev_id, 
            &lenght, (const unsigned char **)binaries, &binary_status, &status);
    checkError(binary_status);
    ...
    cl_kernel k_round0 = clCreateKernel(program, "kernel_round0", &status);
    if (status != CL_SUCCESS || !k_round0)
...
int main(int argc, char *argv[])
{
    ...
    while (-1 != (i = getopt_long_only(argc, argv, "", optlong, 0))) {
        switch(i) {
            ...
            case OPT_LIST:
                ...
                break;
            case OPT_USE:
                device_to_use = parse_num(optarg);
                break;
            case OPT_MINING:
                mining = 1;
                break;
    ...
    printf("Revision opencl for FPGA\n");
    tests();
    if (mining)
